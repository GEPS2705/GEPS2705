# Hi there, I'm Gerardo Puente  👋

### Civil Engineer & Construction Manager | Transitioning to Data-Driven Project Management

I am a Senior Professional with 15 years of experience managing complex construction projects and budgets. Currently, I am bridging the gap between physical infrastructure and digital intelligence by integrating Data Analytics and Agile Project Management into the construction lifecycle.

---

### 🏗️ Domain Expertise & Transition
- **Industry Veteran:** 15 years in Construction Management & Civil Engineering.
- **Academic Foundation:** Master's Degree in Construction Management.
- **The Tech Pivot:** Currently completing the **IBM Data Analyst Professional Certificate** to complement my **Google Project Management** and **Google IT Support** credentials.
- **Goal:** Leveraging data-driven insights to optimize scheduling, resource allocation, and cost-efficiency in the ConTech (Construction Technology) sector.

---

### 🛠️ Tech Stack & Tools
- **Data Analysis:** ![Excel](https://img.shields.io/badge/Microsoft_Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white) (Advanced), ![SQL](https://img.shields.io/badge/sql-4479A1?style=for-the-badge&logo=postgresql&logoColor=white) (In-progress), ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) (In-progress).
- **Project Management:** ![Agile](https://img.shields.io/badge/Agile-0052CC?style=for-the-badge&logo=jira&logoColor=white), ![Scrum](https://img.shields.io/badge/Scrum-636363?style=for-the-badge&logo=scrumalliance&logoColor=white), ![Lean](https://img.shields.io/badge/Lean_Construction-D32F2F?style=for-the-badge&logo=lean&logoColor=white).
- **Visualization:** ![IBM_Cognos](https://img.shields.io/badge/IBM_Cognos-052FAD?style=for-the-badge&logo=ibm&logoColor=white), Dashboards, BIM (Building Information Modeling).
- **Infrastructure:** IT Support, System Troubleshooting.
- **Certifications:** ![Google_PM](https://img.shields.io/badge/Google_Project_Management-4285F4?style=for-the-badge&logo=google&logoColor=white), ![Google_IT](https://img.shields.io/badge/Google_IT_Support-34A853?style=for-the-badge&logo=google&logoColor=white), ![IBM_Data_Analyst](https://img.shields.io/badge/IBM_Data_Analyst-052FAD?style=for-the-badge&logo=ibm&logoColor=white).

---

### 📈 What I'm Working On
- 🔭 **Currently Learning:** Data cleaning and visualization techniques using Python and SQL.
- 🏗️ **Side Project:** Developing a data dashboard to analyze construction budget variances (Coming Soon).
- 🎓 **Education:** Columbia University Specialization in Construction Management (Coursera).

---

### 📫 Connect with me
- [LinkedIn]: https://www.linkedin.com/in/gerardopuente/
- [Portfolio/Website]: [Under_Construction]

*"Transforming physical landscapes through digital insights."*
